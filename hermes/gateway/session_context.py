...
# ---------------------------------------------------------------------------
# External session-source registry (ASOST extension — additive only).
#
# Allows an embedding host (e.g. ASOST) to register itself as the authoritative
# origin of the session id, instead of the native Hermes platforms
# (CLI / gateway / cron). When a provider is registered and the ContextVar for
# ``HERMES_SESSION_ID`` is _UNSET in this context, :func:`get_session_env`
# resolves the id through the registered getter *before* falling back to
# ``os.environ``. If no provider is registered (or the getter raises), behavior
# is byte-for-byte identical to the original implementation.
# ---------------------------------------------------------------------------
from typing import Callable as _Callable

_SESSION_SOURCE_PROVIDER_NAME: str | None = None
_SESSION_ID_GETTER: _Callable[[], str] | None = None


def register_session_source(
    provider_name: str,
    session_id_getter: _Callable[[], str],
) -> None:
    """Register an external host as the authoritative session-id source.

    ``session_id_getter`` must be a zero-argument callable returning the
    current session id string (e.g. ASOST's active-task id). It is consulted
    by :func:`get_session_env("HERMES_SESSION_ID", ...)` whenever no session
    was bound in this context via ``set_session_vars`` /
    ``set_current_session_id``.

    Passing ``None`` as ``session_id_getter`` unregisters the provider and
    restores the default Hermes-native resolution order.
    """
    global _SESSION_SOURCE_PROVIDER_NAME, _SESSION_ID_GETTER
    if session_id_getter is None:
        _SESSION_SOURCE_PROVIDER_NAME = None
        _SESSION_ID_GETTER = None
        return
    _SESSION_SOURCE_PROVIDER_NAME = provider_name
    _SESSION_ID_GETTER = session_id_getter


def unregister_session_source() -> None:
    """Remove any registered external session source (restore defaults)."""
    register_session_source(None, None)


def get_registered_session_source():
    """Return ``(provider_name, session_id_getter)`` or ``(None, None)``."""
    return _SESSION_SOURCE_PROVIDER_NAME, _SESSION_ID_GETTER
...
def get_session_env(name: str, default: str = "") -> str:
    ...
    2. The externally-registered session source (see
       :func:`register_session_source`) — consulted only for
       ``HERMES_SESSION_ID`` when the context variable was never set here.
    3. ``os.environ`` (only when the context variable was never set in
    ...
    if var is not None:
        ...
        if value is not _UNSET:
            return value
    # Externally-registered source (e.g. ASOST tasks) takes precedence over
    # the process-global env mirror; errors in the getter degrade silently to
    # the original fallback chain so hosts can't break Hermes tools.
    if name == "HERMES_SESSION_ID" and _SESSION_ID_GETTER is not None:
        try:
            resolved = _SESSION_ID_GETTER()
        except Exception:
            resolved = None
        if resolved:
            return str(resolved)
